...
def lambda_handler(event, context):
    ...
    logger.info("Processing Job Name: %s", job_name)
    logger.info("Processing Job Definition: %s", processing_job_definition)
    ...
    # Build the create_processing_job kwargs, omitting empty inputs/outputs
    # SageMaker rejects ProcessingOutputConfig with an empty Outputs list
    create_kwargs = dict(
    ...
    if args.get("ProcessingInputs"):
        create_kwargs["ProcessingInputs"] = args["ProcessingInputs"]
    if args.get("ProcessingOutputConfig", {}).get("Outputs"):
        create_kwargs["ProcessingOutputConfig"] = args["ProcessingOutputConfig"]

    # Create the SageMaker processing job
    response = sagemaker_client.create_processing_job(**create_kwargs)
