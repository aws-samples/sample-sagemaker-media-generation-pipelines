...
class ClusterConfig(BaseModel):
    ...
    InstanceType: str = Field(
        description="AWS ML instance type for the processing job"
    )
    VolumeSizeInGB: int = Field(
        ge=50, 
        le=125,
        description="EBS volume size in GB (125 is the maximum)"
    )
...
class AppSpecification(BaseModel):
    ...
    Attributes:
        ...
        ContainerArguments: Arguments to pass to the container
    """
    ImageUri: str = Field(
        description="URI of the Docker image for the processing job"
    )
...
class S3Input(BaseModel):
    ...
    LocalPath: str = Field(
        description="Local path where data will be mounted in the container"
    )
    S3DataType: str = Field(
        description="Type of S3 data (S3Prefix, ManifestFile, etc.)"
    )
    S3InputMode: str = Field(
        description="How data is provided to container (File, Pipe)"
    )
...
class ProcessingInput(BaseModel):
    ...
    """
    InputName: str = Field(
        description="Name identifier for this input"
    )
    S3Input: S3Input
...
class S3Output(BaseModel):
    ...
    """
    S3Uri: str = Field(
        description="S3 URI where output will be stored"
    )
    LocalPath: str = Field(
        description="Local path in container where output is written"
    )
    S3UploadMode: str = Field(
        description="When to upload output (Continuous, EndOfJob)"
    )
...
class StoppingCondition(BaseModel):
    ...
    MaxRuntimeInSeconds: int = Field(
        description="Maximum runtime in seconds before job is stopped"
    )
...
class VpcConfig(BaseModel):
    ...
    """
    SecurityGroupIds: List[str] = Field(
        description="List of security group IDs for network access"
    )
...
class NetworkConfig(BaseModel):
    ...
    """
    EnableInterContainerTrafficEncryption: bool = Field(
        description="Enable encryption for inter-container traffic"
    )
    EnableNetworkIsolation: bool = Field(
        description="Enable network isolation for the processing job"
    )
    VpcConfig: VpcConfig
...
class Arguments(BaseModel):
    ...
    RoleArn: str = Field(
        description="IAM role ARN for the processing job"
    )
    ProcessingInputs: List[ProcessingInput] = Field(
...
class ProcessingDefinition(BaseModel):
    ...
    Type: str = Field(
        description="Type of the job (typically 'Processing')"
    )
    Arguments: Arguments
    DependsOn: Optional[List[str]] = Field(
        default=None,
        description="Optional list of job dependencies"
    )
